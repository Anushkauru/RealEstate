...
<html lang="en">
  <head>
    <title>Client Material &mdash;</title>
    <meta charset="utf-8">
	...
	function validateForm() {
		//get the value of the "CustomerAadharNumber" element within the "formName"            
	var x = document.forms["formName"]["CustomerAadharNumber"].value;
	//if CustomerAadharNumber field is empty
	if (x == "") {
		alert("Aadhar Number must be filled out");
		return false;
	}
	// get the value of the "materialadmin" element within the "formName"            
	var y = document.forms["formName"]["materialadmin"].value;
		//if materialadmin field is empty
	if (y == "") {
		alert("Manufacturer must be selected");
		return false;
	}
	// get the value of the "materialclient" element within the "formName"            
	//var z = document.forms["formName"]["materialclient"].value;
		//if materialclient field is empty

	//if (z == "") {
		//alert("Material Name must be selected");
		//return false;
	//}
	...
	if (q == "") {
		...
		{//to update values in table automatically
				$("#materialadmin").change(function(){
					var mid= $("#materialadmin").val();
					$.ajax({
						url: 'data.php',
						method: 'post',
						data: 'mid=' +mid
					}).done(function(materialclient){
						console.log(materialclient);
						materialclient = JSON.parse(materialclient);
						$('#materialclient').empty();
						materialclient.forEach(function(materialclient1){
							$('#materialclient').append('<option>' + materialclient1.MaterialName + '</option>')
						})
					})
				})
		})
    ...
    { 
	//if aahar number is not 12 digits or empty or not number 
...
<body>
<div id="login">
<form action="ClientMaterialConnection.php" method="POST" name="formName" onsubmit="return validateForm()">
<fieldset >
<br><br>
<p><input type="text" placeholder="Aadhar Number" name="CustomerAadharNumber">

</p>

<p2><label for="materialadmin"> Manufacturers</label></p2><br>
<p><select name="materialadmin" id="materialadmin">
...
<p><input type="submit" value="Submit" name="SubmitButton"></p>
    ...
    <footer class="site-footer">
      <div class="container">
		...
		All rights reserved | <a href="http://www.onego.in" target="_blank" >OneGo</a>
